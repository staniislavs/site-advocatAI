...
const SchemaOrg: React.FC<{ data: any }> = ({ data }) => (
  <script
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-9 gap-3">
            <Link 
              ...
              className={`flex flex-col items-center justify-center p-4 sm:p-6 rounded-3xl border transition-all group ${!category ? 'bg-gold border-gold text-white font-bold shadow-lg shadow-gold/20' : 'bg-[var(--bg-secondary)] border-[var(--card-border)] hover:border-gold/40'}`}
            >
            ...
            {BLOG_CATEGORIES.map(cat => (
              <Link 
                ...
                className={`flex flex-col items-center justify-center p-4 sm:p-6 rounded-3xl border transition-all group ${category === cat.id ? 'bg-gold border-gold text-white font-bold shadow-lg shadow-gold/20' : 'bg-[var(--bg-secondary)] border-[var(--card-border)] hover:border-gold/40'}`}
              >
...
const BlogPostView: React.FC<{ 
  ...
  const breadcrumbSchema = {
    ...
    "@type": "BreadcrumbList",
    "itemListElement": [
      { "@type": "ListItem", "position": 1, "name": "Головна", "item": `${window.location.origin}/${currentLang}` },
      { "@type": "ListItem", "position": 2, "name": "Блог", "item": `${window.location.origin}/${currentLang}/${paths.blog}` },
      { "@type": "ListItem", "position": 3, "name": getCategoryLabel(post.category, t), "item": `${window.location.origin}/${currentLang}/${paths.blog}/${post.category}` },
      { "@type": "ListItem", "position": 4, "name": post.title, "item": window.location.href }
    ]
  };
  ...
  const articleSchema = {
    ...
    "dateModified": post.updatedAt?.toDate?.() || post.updatedAt || post.createdAt,
    "author": [{
      "@type": "Person",
      "name": post.author,
      "url": `${window.location.origin}/${currentLang}/${paths.about}`
    }]
  };
  ...
  return (
    <div className="bg-[var(--bg-primary)] min-h-screen">
      ...
      {faqSchema && <SchemaOrg data={faqSchema} />}
