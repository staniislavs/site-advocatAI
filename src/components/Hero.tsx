  ...
  return (
    <section id="hero" ref={targetRef} className="relative min-h-[110vh] flex items-center overflow-hidden pt-20">
      ...
      <motion.div 
        ...
        className="absolute top-[-20%] left-[-10%] w-[60%] h-[80%] rounded-full bg-gold/3 blur-[120px] pointer-events-none z-[1]" 
      />
      <motion.div 
        style={{ y: useTransform(scrollYProgress, [0, 1], [0, 200]) }}
        className="absolute bottom-[-10%] right-[-5%] w-[50%] h-[70%] rounded-full bg-gold/5 blur-[100px] pointer-events-none z-[1]" 
      />
      ...
      <div className="max-w-7xl mx-auto px-6 w-full grid lg:grid-cols-2 gap-12 lg:gap-20 relative z-10">
        <motion.div style={{ y, opacity }} className="flex flex-col justify-center text-center lg:text-left items-center lg:items-start">
          <motion.div 
            ...
            className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-gold/5 border border-gold/10 text-gold text-[9px] font-bold uppercase tracking-[0.3em] mb-8 w-fit"
          >
          ...
          <h1 className="font-serif text-4xl sm:text-5xl md:text-6xl lg:text-8xl text-[var(--text-primary)] leading-[1.05] mb-8 md:mb-10 tracking-tight">
            ...
            <Reveal text={t('hero.title_2_italic')} className="text-gold italic font-light" delay={0.4} />
          </h1>
          ...
          >
            <Link 
              ...
              className="bg-gold hover:bg-gold-light text-navy-deep font-bold px-8 py-4 md:px-10 md:py-5 rounded-full transition-all group flex items-center justify-center gap-3 shadow-xl shadow-gold/20 active:scale-95 text-sm md:text-base"
            >
              {t('hero.cta_free')}
            ...
            <a 
              ...
              target="_blank" 
              rel="noreferrer"
              className="border border-[var(--card-border)] hover:border-gold/50 text-[var(--text-primary)] px-8 py-4 md:px-10 md:py-5 rounded-full transition-all flex items-center justify-center gap-3 active:scale-95 text-sm md:text-base h-full"
            >
              Telegram
          ...
          <motion.div 
            ...
            className="flex flex-wrap justify-center lg:justify-between gap-x-12 gap-y-8 py-10 border-t border-gold/10 w-full"
          >
            <div className="flex flex-col items-center lg:items-start text-center lg:text-left">
              <div className="font-serif text-3xl sm:text-4xl text-gold pb-1">{t('hero.stats.practice_val', { years: experienceYears })}</div>
              <div className="text-[10px] uppercase tracking-[0.3em] text-[var(--text-muted)] font-bold">{t('hero.stats.practice')}</div>
            </div>
            <div className="flex flex-col items-center lg:items-start text-center lg:text-left">
              <div className="font-serif text-3xl sm:text-4xl text-gold pb-1">{t('hero.stats.won_val')}</div>
              <div className="text-[10px] uppercase tracking-[0.3em] text-[var(--text-muted)] font-bold">{t('hero.stats.won')}</div>
            </div>
            <div className="flex flex-col items-center lg:items-start text-center lg:text-left">
              <div className="font-serif text-3xl sm:text-4xl text-gold pb-1">{t('hero.stats.response_val')}</div>
      ...
      >
        <span className="text-[9px] font-bold uppercase tracking-[0.4em] text-gold">{t('hero.scroll')}</span>
