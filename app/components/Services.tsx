...
export default function Services() {
  ...
  const services = [
    ...
    { id: 'property', iconKey: 'property', featured: true },
    { id: 'alimony', iconKey: 'alimony', featured: true },
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          <div className="max-w-2xl">
            <div className="inline-flex items-center gap-3 text-sage text-[10px] font-bold uppercase tracking-[0.3em] mb-6">
            ...
            <h2 className="font-serif text-3xl sm:text-4xl md:text-7xl text-[var(--text-primary)] leading-[1.05]">
              {t('services.title')}<span className="text-sage italic font-light">{t('services.title_italic')}</span>
            </h2>
          </div>
        ...
        <motion.div
          ...
          className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-12 gap-4 md:gap-5 lg:gap-4 auto-rows-auto md:auto-rows-fr lg:auto-rows-[100px]"
        >
          {services.map((service, idx) => {
            const isFeatured = service.featured;
            const isMainFeatured = idx === 0; // Divorce - main featured
            const isSecondaryFeatured = idx === 1; // Property - tall featured
            const isTertiaryFeatured = idx === 2; // Alimony - medium featured
            const isMedium = idx >= 3 && idx <= 4;
            const isWide = idx >= 5;

            let gridClasses = '';
            // Mobile: featured cards span 2 columns, non-featured span 1
            // Desktop: restored original hierarchy
            if (isMainFeatured) {
              gridClasses = 'col-span-2 md:col-span-4 lg:col-span-8 lg:row-span-3 min-h-[200px] md:min-h-[250px] lg:min-h-[300px] bg-[var(--card-bg)] text-[var(--text-primary)] border-sage/20 shadow-2xl';
            } else if (isSecondaryFeatured) {
              gridClasses = 'col-span-2 md:col-span-2 lg:col-span-4 lg:row-span-3 min-h-[200px] md:min-h-[220px] lg:min-h-auto bg-[var(--card-bg)] border-sage/20 shadow-lg';
            } else if (isTertiaryFeatured) {
              gridClasses = 'col-span-2 md:col-span-2 lg:col-span-4 lg:row-span-2 min-h-[200px] md:min-h-[220px] lg:min-h-auto bg-[var(--card-bg)] border-sage/20 shadow-lg';
            } else if (isMedium) {
              gridClasses = 'col-span-1 md:col-span-2 lg:col-span-4 lg:row-span-2 bg-[var(--card-bg)] border-[var(--card-border)]';
            } else {
              gridClasses = 'col-span-1 md:col-span-2 lg:col-span-6 lg:row-span-2 bg-[var(--card-bg)] border-[var(--card-border)]';
            }
            ...
            return (
              <motion.div
                ...
                className={`group cursor-pointer flex flex-col justify-between p-4 md:p-5 rounded-[1.5rem] border transition-all duration-500 relative overflow-hidden
                  hover:border-sage/40 shadow-[0_8px_30px_rgba(0,0,0,0.02)] hover:shadow-[0_20px_50px_rgba(94,105,77,0.1)]
                  ${gridClasses}
                `}
              >
                <div className={`absolute top-0 right-0 w-32 h-32 rounded-bl-[80px] translate-x-12 -translate-y-12 group-hover:translate-x-6 group-hover:-translate-y-6 transition-transform duration-700
                  ${isFeatured ? 'bg-sage/10' : 'bg-sage/5'}
                `} />
                ...
                <div className="relative z-10 flex flex-col h-full w-full">
                  <div className={`mb-3 md:mb-4 w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-[1rem] flex items-center justify-center transition-all duration-700 relative
                    ${isFeatured 
                      ? 'bg-sage text-[var(--bg-primary)] shadow-[0_10px_30px_rgba(94,105,77,0.4)]' 
                      : 'bg-sage/10 text-sage group-hover:bg-sage group-hover:text-white shadow-sm group-hover:shadow-[0_15px_35px_rgba(94,105,77,0.4)]'}
                  `}>
                    {/* Inner glow effect */}
                    {!isFeatured && (
                      <div className="absolute inset-0 rounded-xl md:rounded-[1rem] bg-sage/20 blur-md opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none" />
                    )}
                  ...
                  <div className="flex-grow">
                    <h3 className={`font-serif mb-1 md:mb-2 leading-[1.1] transition-all duration-300
                      ${isFeatured ? 'text-xl md:text-[30px] text-[var(--text-primary)]' : 'text-lg md:text-[27px] text-[var(--text-primary)] group-hover:text-sage'}
                    `}>
                      {service.name}
                    </h3>
                  ...
                  <div className={`flex items-center justify-between pt-4 md:pt-6 mt-auto border-t
                    ${isFeatured ? 'border-sage/10' : 'border-[var(--card-border)]'}
                  `}>
                    <span className={`text-[9px] md:text-[10px] font-bold uppercase tracking-[0.2em] transition-colors
                      ${isFeatured ? 'text-sage' : 'text-[var(--text-muted)] group-hover:text-sage'}
                    `}>
                      {t('services.more')}
                    </span>
                    <div className={`w-7 h-7 md:w-8 md:h-8 rounded-full border flex items-center justify-center transition-all
                      ${isFeatured ? 'border-sage/30 bg-sage/5 group-hover:bg-sage group-hover:border-sage' : 'border-[var(--card-border)] group-hover:border-sage group-hover:bg-sage/5'}
                    `}>
                      <ArrowUpRight size={14} className={isFeatured ? 'text-sage group-hover:text-navy' : 'text-[var(--text-muted)] group-hover:text-sage'} />
                    </div>
                  </div>
                </div>
              </motion.div>
        ...
        {selectedService && (
          ...
          >
            ...
            <motion.div
              ...
              className="relative w-full max-w-5xl bg-[var(--bg-secondary)] rounded-[2rem] md:rounded-[3rem] border border-sage/20 shadow-[0_50px_100px_rgba(0,0,0,0.3)] overflow-hidden flex flex-col md:flex-row max-h-[90vh] md:max-h-none cursor-pointer lg:cursor-default"
            >
              {/* Image/Visual Side */}
              <div className="w-full md:w-2/5 bg-[var(--bg-primary)] relative overflow-hidden p-6 md:p-16 flex flex-col justify-between border-b md:border-b-0 md:border-r border-sage/10">
                <div className="absolute top-0 right-0 w-48 md:w-64 h-48 md:h-64 bg-sage/5 blur-[80px] md:blur-[100px] -translate-y-1/2 translate-x-1/4 rounded-full" />
                
                <motion.button 
                  whileHover={{ scale: 1.1, rotate: 90 }}
                  whileTap={{ scale: 0.9 }}
                  onClick={() => setSelectedService(null)}
                  className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-[var(--text-primary)]/5 border border-[var(--text-primary)]/10 flex items-center justify-center text-[var(--text-primary)]/40 hover:text-sage hover:border-sage/50 transition-all mb-6 md:mb-12 self-start"
                >
                  <X size={18} />
                </motion.button>
                
                <div>
                   <div className="w-12 h-12 md:w-20 md:h-20 rounded-2xl md:rounded-3xl bg-sage text-[var(--bg-primary)] flex items-center justify-center mb-6 md:mb-8 shadow-2xl shadow-sage/20">
              ...
              <div className="w-full md:w-3/5 p-6 md:p-16 bg-[var(--bg-secondary)] overflow-y-auto">
                 <h4 className="text-sage text-[10px] md:text-xs font-bold uppercase tracking-widest mb-6 md:mb-8 flex items-center gap-2">
                   <span className="w-4 md:w-6 h-[1px] bg-sage" />
                   {t('services.more')}
                 </h4>
                 ...
                 <div className="space-y-4 md:space-y-6">
                   {Array.isArray(selectedService.details) && selectedService.details.map((detail: string, i: number) => (
                     <motion.div 
                      ...
                      >
                       <CheckCircle2 className="text-sage mt-1 flex-shrink-0" size={18} />
                 ...
                 <div className="mt-10 md:mt-16 flex flex-col sm:flex-row gap-4">
                   ...
                   <Link 
                     ...
                     className="inline-flex items-center justify-center gap-3 bg-sage hover:bg-[#a6844d] text-[#0a111f] font-bold px-8 py-4 md:px-10 md:py-5 rounded-full text-[10px] md:text-xs uppercase tracking-[0.2em] transition-all active:scale-95 shadow-xl shadow-sage/30"
                   >
