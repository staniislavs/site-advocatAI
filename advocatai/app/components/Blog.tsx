            ...
            {!category && (
              <aside className="w-full lg:w-80 space-y-8">
                ...
                <div className="bg-sage p-8 rounded-3xl text-navy-deep">
                  ...
                  <button onClick={() => navigate(`/${currentLang}/${paths.contacts}`)} className="w-full bg-navy-deep text-sage py-3 rounded-xl font-bold uppercase text-[10px] tracking-widest">
                    Записатися
                  </button>
                </div>
              </aside>
          ...
          <div className="relative z-10">
            ...
            <form className="space-y-5" onSubmit={handleCtaSubmit}>
              ...
              <button 
                type="submit"
                disabled={submitting}
                className="w-full bg-sage text-white py-5 rounded-[1.2rem] font-bold uppercase text-[11px] tracking-[0.2em] hover:bg-[#a6844d] transition-all shadow-xl shadow-sage/20 disabled:opacity-50 flex items-center justify-center gap-3 active:scale-[0.98]"
              >
    ...
    <div className="fixed inset-0 z-[110] flex flex-col items-center justify-start p-4 md:p-10 overflow-hidden">
      ...
      >
                ...
                <div className="flex gap-4">
                  {['markdown', 'html'].map(type => (
                    <button 
                      ...
                      className={`flex-1 py-3 rounded-xl text-xs font-bold border transition-all ${post.contentType === type ? 'bg-sage text-navy-deep border-sage' : 'border-sage/20 text-sage hover:bg-sage/5'}`}
                    >
                      {type.toUpperCase()}
                    </button>
                  ))}
        ...
        <div className="mt-8 flex flex-col md:flex-row gap-4 flex-shrink-0">
          <button 
            onClick={onSave}
            className="flex-1 bg-sage text-navy-deep px-12 py-4 rounded-2xl font-bold uppercase text-sm tracking-widest shadow-xl shadow-sage/20 hover:scale-[1.02] active:scale-95 transition-all"
          >
            Зберегти зміни
          </button>
          <button 
