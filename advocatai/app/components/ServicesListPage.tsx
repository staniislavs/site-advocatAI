    ...
    <div className="pt-32 pb-32">
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          <button 
            onClick={() => navigate(-1)}
            className="group flex items-center gap-3 text-[10px] font-bold uppercase tracking-[0.3em] text-[var(--text-muted)] hover:text-sage transition-all"
          >
        ...
        >
          <div className="max-w-3xl">
            ...
            <h1 className="font-serif text-5xl md:text-8xl text-[var(--text-primary)] leading-[1.05] tracking-tight">
              {t('services.title')}<br />
              <span className="text-sage italic font-light">{t('services.title_italic')}</span>
            </h1>
          </div>
        ...
        >
          {services.map((service, idx) => {
            ...
            let gridClasses = '';
            if (isFeatured) gridClasses = 'md:col-span-4 lg:col-span-8 lg:row-span-3 min-h-[200px] md:min-h-[250px] lg:min-h-[300px] bg-[var(--card-bg)] text-[var(--text-primary)] border-sage/20 shadow-2xl';
              ...
              >
                ...
                <div className="relative z-10 flex flex-col h-full w-full">
                  ...
                  `}>
                    ...
                    `}>
                      <ArrowUpRight size={14} className={isFeatured ? 'text-sage group-hover:text-white' : 'text-[var(--text-muted)] group-hover:text-sage'} />
                    </div>
                  </div>
                </div>
              </motion.div>
        ...
        >
          <Link 
            to={`/${currentLang}`} 
            className="group inline-flex items-center gap-4 bg-[var(--bg-secondary)] border border-[var(--card-border)] rounded-full px-8 py-4 text-[10px] font-bold uppercase tracking-[0.3em] text-[var(--text-muted)] hover:text-sage hover:border-sage/30 hover:shadow-[0_20px_50px_rgba(0,0,0,0.2)] transition-all duration-500 shadow-sm"
          >
            <div className="w-6 h-6 rounded-full border border-[var(--card-border)] flex items-center justify-center group-hover:border-sage group-hover:bg-sage/5 transition-all">
        ...
        {selectedService && (
          ...
          >
            ...
            <motion.div
              ...
              className="relative w-full max-w-5xl bg-[var(--bg-secondary)] rounded-[2rem] md:rounded-[3rem] border border-sage/20 shadow-[0_50px_100px_rgba(0,0,0,0.3)] overflow-hidden flex flex-col md:flex-row max-h-[90vh] md:max-h-none"
            >
              <div className="w-full md:w-2/5 bg-[var(--bg-primary)] relative overflow-hidden p-8 md:p-16 flex flex-col justify-between border-b md:border-b-0 md:border-r border-sage/10">
                ...
                <motion.button 
                  ...
                  whileTap={{ scale: 0.9 }}
                  onClick={() => setSelectedService(null)}
                  className="w-12 h-12 rounded-full bg-[var(--text-primary)]/5 border border-[var(--text-primary)]/10 flex items-center justify-center text-[var(--text-primary)]/40 hover:text-sage hover:border-sage/50 transition-all mb-8 self-start"
                >
                  <X size={20} />
                </motion.button>
                
                <div>
                  <div className="w-16 h-16 md:w-24 md:h-24 rounded-3xl bg-sage text-[var(--bg-primary)] flex items-center justify-center mb-8 shadow-2xl shadow-sage/20">
              ...
              <div className="w-full md:w-3/5 p-8 md:p-16 bg-[var(--bg-secondary)] overflow-y-auto">
                 <h4 className="text-sage text-[10px] md:text-xs font-bold uppercase tracking-widest mb-8 flex items-center gap-3">
                 ...
                 <div className="space-y-6">
                   {Array.isArray(selectedService.details) && selectedService.details.map((detail: string, i: number) => (
                     <motion.div 
                      ...
                      >
                       <CheckCircle2 className="text-sage mt-1 flex-shrink-0" size={20} />
                 ...
                 <div className="mt-16 flex flex-col sm:flex-row gap-6">
                   ...
                   <Link 
                     ...
                     onClick={() => setSelectedService(null)}
                     className="inline-flex items-center justify-center gap-3 bg-sage hover:bg-[#a6844d] text-white font-bold px-10 py-5 rounded-full text-[10px] md:text-xs uppercase tracking-[0.2em] transition-all active:scale-95 shadow-xl shadow-sage/30"
                   >
